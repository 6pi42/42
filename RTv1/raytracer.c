/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   raytracer.c                                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: Client <Client@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/22 16:47:16 by cboyer            #+#    #+#             */
/*   Updated: 2016/02/27 20:00:56 by Client           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

t_vec    init_ray(t_map *map, int x, int y)
{
	t_vec   ray;
	ray.x = (x - map->tab->screen.x / 2) * 1;
	ray.y = (y - map->tab->screen.y / 2) * 1;
	ray.z = -(map->tab->screen.x / (2 * tan((90 / 2) * M_PI / 180.0)));
	return (ray);
}

int		smaller_void(void *st)
{
	int	i;
	int	tmp;
	int	j;

	i = 0;
	tmp = -1;
	j = -1;
	while (i < 1)
	{
		if (tmp > *(double*)st[i] && *(double*)st[i] != -1)
		{
			tmp = *(double*)st[i];
			j = i;
		}
		i++;
	}
	if (tmp == -1)
		return (-1);
	return (*(int*)st[i] + sizeof(double));
}

void	reset(void *st)
{
	int	i;

	i = 0;
	while (i < 1)
	{
			st[i] = 0;
			i++;
	}
}

void	raytracer(t_map *map)
{
	int		y;
	int		x;
	void	*st[1];

	reset(st);
	y = 0;
	while (y < map->tab->screen.y)
	{
		x = 0;
		while (x < map->tab->screen.x)
		{
			if (tab->map->nb_sphere)
				st[0] = nearest_sphere(y, x, map, map->tab->sphere);
			st[1] = plan(i, j, map);
			if (st[0] != NULL && *(double*)stru[0] != -1)
				pixel_put(map, x, y, *(int*)(stru[0] + sizeof(double)));
			reset(st);
			j++;
		}
		i++;
	}
}
